...
function MultiLightbox({ images, onClose }: { images: { src: string; alt: string }[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      <p className="text-xs text-gray-400 mt-2">{idx + 1} / {images.length}</p>
    </div>
  )
}
...
function Shot({ src, alt, label, desc, className = '' }: { src: string; alt: string; label: string; desc: string; className?: string }) {
  ...
  return (
    <>
      <div className={`rounded-xl overflow-hidden border border-white/10 bg-white/5 hover:bg-white/10 transition-all group cursor-pointer ${className}`} onClick={() => setOpen(true)}>
        ...
        <div className="px-3 py-2.5">
          <p className="font-semibold text-sm text-orange-300">{label}</p>
  ...
  return (
    <>
      <div className="rounded-xl overflow-hidden border border-white/10 bg-white/5 hover:bg-white/10 transition-all group cursor-pointer" onClick={() => setOpen(true)}>
        ...
        <div className="px-3 py-2.5">
          <p className="font-semibold text-sm text-orange-300">Tableau de bord</p>
          <p className="text-xs text-gray-400 mt-0.5">9 modules de gestion (2 photos)</p>
        </div>
      </div>
      {open && (
        <MultiLightbox
    ...
    <section className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="p-8 md:p-12 mb-10 flex flex-col items-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {[
            ['Java 17', 'bg-orange-500/20 text-orange-300 border-orange-500/30'],
            ['JavaFX', 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30'],
            ['Spring Boot', 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30'],
            ['Hibernate ORM', 'bg-amber-500/20 text-amber-300 border-amber-500/30'],
            ['PostgreSQL', 'bg-sky-500/20 text-sky-300 border-sky-500/30'],
            ['Material Design', 'bg-pink-500/20 text-pink-300 border-pink-500/30'],
          ].map(([name, cls], i) => (
      ...
      <div className="space-y-10">
        ...
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #10b981' }}>
          <h2 className="text-xl font-bold mb-2" style={{ color: '#34d399' }}>&#128218; Contexte</h2>
          <p className="leading-relaxed">
            La société <strong style={{ color: '#34d399' }}>IMMOSYNC</strong> est un syndic de gestion de biens immobiliers.
            Le projet a été réalisé en <strong style={{ color: '#34d399' }}>équipe de 3</strong> en 2e année de BTS SIO SLAM.
            J'ai travaillé sur la <strong style={{ color: '#34d399' }}>partie Java Desktop</strong> : le back-office du gestionnaire
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #818cf8' }}>
            <h2 className="text-xl font-bold mb-3" style={{ color: '#a5b4fc' }}>&#128196; Fonctionnalités</h2>
            <ul className="grid grid-cols-1 gap-2">
              {[
                ...
                <li key={i} className="flex items-start gap-2 text-sm">
                  <span className="mt-0.5" style={{ color: '#34d399' }}>&#10003;</span>{item}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #a78bfa' }}>
            <h2 className="text-xl font-bold mb-3" style={{ color: '#c4b5fd' }}>&#127919; Objectifs</h2>
        ...
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #60a5fa' }}>
          <h2 className="text-xl font-bold mb-3" style={{ color: '#93c5fd' }}>&#128736;&#65039; Stack technique</h2>
        ...
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #f97316' }}>
          <h2 className="text-xl font-bold mb-1" style={{ color: '#fb923c' }}>&#128248; Démonstration</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #fbbf24' }}>
            <h2 className="text-xl font-bold mb-3" style={{ color: '#fcd34d' }}>&#9888;&#65039; Défis rencontrés</h2>
          ...
          <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #10b981' }}>
            <h2 className="text-xl font-bold mb-3" style={{ color: '#34d399' }}>&#127942; Résultats</h2>
        ...
        {/* Autocritique */}
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm" style={{ borderLeft: '4px solid #a78bfa' }}>
          <h2 className="text-xl font-bold mb-3" style={{ color: '#c4b5fd' }}>&#128172; Ce que ce projet m'a apporté</h2>
          <p className="leading-relaxed">
            Ce projet m'a fait découvrir l'intégration <strong style={{ color: '#a78bfa' }}>Spring Boot + JavaFX</strong>.
            Comme pour la partie web, on a eu des soucis d'organisation à 3 et des <strong style={{ color: '#a78bfa' }}>conflits sur la BDD partagée</strong> entre Hibernate et Doctrine.
            Ça m'a apporté une vraie compétence en <strong style={{ color: '#a78bfa' }}>développement desktop Java</strong> et m'a montré comment deux technologies différentes peuvent coexister sur un même projet.
          </p>
        </div>
