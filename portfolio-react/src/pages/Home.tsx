...
import { Moon, Palette, Stars, Type } from 'lucide-react'
...
export default function Home() {
  ...
  return (
    <section className="section">
      <div ref={ref} className={`grid items-center gap-8 md:grid-cols-2 reveal ${inView ? 'in-view' : ''}`}>
        <div className="order-2 md:order-1">
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">
            Levana Wizman — <span className="heading-gradient">BTS SIO – SLAM | Alternance chez 2C Démarches</span>
          </h1>
          <p className="mt-4 text-gray-600">
            Paris / Vincennes • levanawizman25@gmail.com
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge className="badge-pop">Étudiante BTS SIO – SLAM</Badge>
            <Badge className="badge-pop" variant="secondary">Alternance: 2C Démarches</Badge>
            <Badge className="badge-pop" variant="outline">Paris / Vincennes</Badge>
          </div>
          <div className="mt-6 flex gap-3">
            <Button className="btn-shine" onClick={() => window.dispatchEvent(new Event('open-contact'))}>
              Me contacter
            </Button>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <Button onClick={toggleSparks} variant={sparksOn ? 'gradient' : 'glow'} className="rounded-full btn-spark">
              <Stars size={16} /> Étincelles {sparksOn ? 'ON' : 'OFF'}
            </Button>
        ...
        <div className="order-1 md:order-2 flex justify-center relative">
          <div className="soft-spot" />
          <img
            className="h-48 w-48 md:h-64 md:w-64 rounded-full object-cover ring-4 ring-indigo-100 photo-glow"
            src="https://via.placeholder.com/512x512.png?text=Photo"
            alt="Photo de profil"
          />
        </div>
      </div>
      {/* Bouton pour afficher/masquer le mini-jeu */}
      <div className="mt-8">
